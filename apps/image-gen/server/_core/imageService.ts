...
import { assertProductionImageQuotaConfig } from "./quotaPolicy";
...
export function getGeneratorStartupConfig(): {
  ...
  assertProductionOpenAiImageRetryConfig();
  assertProductionImageQuotaConfig();
  return {
...
export class OpenAiImageGenerator implements ImageGenerator {
  async generate(input: GeneratorInput): Promise<{
    ...
    try {
      ...
      const imageRequestOptions = requestContext.imageRequestOptions;
      const payloadBytes =
      ...
      const response = await fetchOpenAiImageResponse(requestContext, {
        ...
        onProviderAttempt: async () => {
          ...
          try {
            ...
            const providerSpendBudget = admission?.providerSpendBudget;
              ...
              await appendCostLedgerEntry(
                {
                  ...
                  estimatedCostUsd:
                    providerSpendBudget?.estimatedCostUsd ?? null,
                  estimatedOutputCostUsd: null,
                  finalCostUsd: null,
                  costEstimateComplete: providerSpendBudget !== undefined,
                  estimateSource: providerSpendBudget?.estimateSource ?? null,
                  unpricedCostComponents: providerSpendBudget
                    ? []
                    : ["image_generation"],
                },
                attemptedAt
            ...
            if (providerSpendBudget) {
              await admitMessengerProviderSpend({
                reqId: input.reqId,
                attemptId: costLedgerEntryId,
                userKey: input.userKey,
                tenantScope: input.costLedgerScope,
                estimatedCostUsd: providerSpendBudget.estimatedCostUsd,
                estimatedOutputCostUsd: null,
