...
| One-time premium image credits      | Reviewed bounded Test exposure; execution pending and actual Machines still checkout-off. After protected activation and deployment, all eligible Messenger users may use this flow without tester registration or customer login. Live billing remains disabled. Only when the free daily image quota is exhausted and the user's separately scoped premium wallet is empty, Leaderbot remains provider-silent and explains the exact free reset plus a no-purchase alternative. It may offer one one-time **EUR 4.99** purchase for **8 non-expiring medium-quality premium image credits**, with no subscription, renewal, mandate, automatic top-up, or overage. The CTA opens a short-lived, signed, single-use `app.leaderbot.live` capability. No Mollie payment is created until the user explicitly confirms on the browser page. Declining creates no checkout and makes no image-provider call. The browser return grants no credits; credits are granted only after the trusted Mollie webhook verifies the payment. | Required E2E evidence remains pending: demo exhaustion with both the free allowance and premium wallet empty, the reset/no-purchase copy, decline, expired/replayed capability, explicit browser confirmation, canceled/failed payment, return without grant, delayed/replayed trusted webhook, and exactly-once grant. Confirm that the CTA is sent only in a user-initiated eligible Messenger conversation under Meta's 24-hour messaging-window policy; an exhausted quota never widens eligibility. | Uses the existing Page messaging permission and a Messenger URL action. No payment credentials are collected in Messenger and no additional Meta permission is expected. |
...
- While waiting for initial processing consent, Messenger holds only the text
  ...
  Refusal and scoped erasure delete it. Its TTL also expires abandoned input
...
10. The quota CTA only inside a user-initiated Meta-eligible 24-hour response
    window. Verify the short-lived signed single-use `app.leaderbot.live`
...
15. A delivered generated image whose post-delivery bookkeeping fails: the
    delivered image stays the only response, no generation-failure copy
    follows, and no second generation, credit, or quota consumption occurs.
    Repeat with a paid send that has only a pending delivery receipt and with a
    provider failure before delivery, and confirm the localized failure copy
    still arrives: with its `Nieuwe afbeelding` retry action in the
    conversation, and as plain text without an action when a queued job is
